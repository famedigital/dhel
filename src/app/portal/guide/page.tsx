...
export default async function PortalGuideHome({
  searchParams,
}: {
  ...
  if (!staff || staff.role !== "guide") {
    ...
    redirect("/portal/login?error=" + encodeURIComponent("No guide profile linked"));
  }
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-[var(--muted-foreground)]">Guide portal</p>
      ...
      <div className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
        ...
        <p className="mt-1 text-xs text-[var(--muted-foreground)]">
          Bank QR: {staff.row.emv_static ? "Set" : "Not set — add in Profile"}
        </p>
      </div>
      ...
      <div className="mt-6 flex gap-2">
        <Link href={`/portal/guide/trips${q}`} className="btn btn-secondary flex-1 text-center">
          All trips
        </Link>
        <Link href={`/portal/guide/profile${q}`} className="btn btn-primary flex-1 text-center">
          Profile
        </Link>
      </div>
    </PortalShell>
