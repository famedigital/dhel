...
function MorphingText({ text }: { text: string }) {
  return <span className="whitespace-nowrap px-0.5">{text}</span>;
}
...
export interface PromptInputProps {
  ...
  /** Full-width dock (Gemini-style) instead of the compact morphing pill. */
  fullWidth?: boolean;
  /** Hide model / effort / attach chrome — cleaner agent desk input. */
  minimal?: boolean;
}
...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(
  (
    ...
    useEffect(() => {
      setContainerHeight(Math.max(minimal ? 88 : 116, textareaHeight + (minimal ? 36 : 48)));
      setTimeout(updateFades, 0);
    }, [textareaHeight, minimal]);
      ...
      <>
        ...
        >
          ...
          >
            ...
            <div
              className={cn(
                "absolute bottom-2 left-3 right-12 z-[10] flex items-center gap-0 transition-[opacity,transform,filter] duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)]",
            ...
            >
              ...
              <button
                ...
                className="ml-auto flex size-7 items-center justify-center rounded-full text-foreground/50 transition-[color,background-color,opacity] duration-200 hover:bg-accent/60 hover:text-foreground outline-none cursor-default disabled:opacity-40 disabled:pointer-events-none"
              >
                <PlusIcon />
              </button>
            </div>
